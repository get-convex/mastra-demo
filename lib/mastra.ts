...
import { LogLevel, createLogger, noopLogger } from "@mastra/core/logger";
import type { Logger } from "@mastra/core/logger";
import type { MastraMemory } from "@mastra/core/memory";
import type { MastraStorage } from "@mastra/core/storage";
import { InstrumentClass, Telemetry } from "@mastra/core/telemetry";
import type { OtelConfig } from "@mastra/core/telemetry";
import type { MastraTTS } from "@mastra/core/tts";
...
`);
    ...
    */
    ...
    if (config?.agents) {
      Object.entries(config.agents).forEach(([key, agent]) => {
        ...
        agent.__registerMastra(this as unknown as MastraCore);
    ...
    */
    ...
    if (config?.workflows) {
      Object.entries(config.workflows).forEach(([key, workflow]) => {
        workflow.__registerMastra(this as unknown as MastraCore);
  ...
  public getVector<TVectorName extends keyof TVectors>(
    name: TVectorName,
...
export { Mastra };
export type { Mastra as MastraCore };
